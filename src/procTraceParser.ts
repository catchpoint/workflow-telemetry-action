...
const SYS_PROCS_TO_BE_IGNORED: Set<string> =
    new Set([
        'awk',
        'basename',
        'cat',
...
export async function parse(filePath: string, procEventParseOptions: ProcEventParseOptions): Promise<CompletedCommand[]> {
    const minDuration: number =
        (procEventParseOptions && procEventParseOptions.minDuration) || -1
    const traceSystemProcesses: boolean =
        (procEventParseOptions && procEventParseOptions.traceSystemProcesses) || false
        ...
        try {
            ...
            if ('EXEC' === event.event) {
                ...
                if (replacedCommand && activeCommandCompleted) {
                    ...
                    const finishTime: number = activeCommand.startTime + activeCommand.duration
    ...
    /*
    console.log('Exec commands count      :', execCommandCount)
    console.log('Exit commands count      :', exitCommandCount)
    console.log('Unknown commands count   :', unknownCommandCount)
    console.log('Completed commands count :', completedCommands.length)
    console.log('Completed commands       :')
    console.log(sprintf(
